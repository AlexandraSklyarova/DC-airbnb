...
# =======================
# Revenue Bar Chart & Bubble Chart (Left)
# Scatter Plot (Right)
# =======================

col1, col2 = st.columns([1.2, 1.0])  # Slightly wider left column

with col1:
    st.header("📊 Revenue by Neighborhood")

    # Sidebar-like controls in main view
    metric = st.selectbox("Select revenue metric:", options=["Average Revenue", "Total Revenue"])
    top_n = st.slider("Select number of neighborhoods to display:", min_value=5, max_value=100, value=30)
...
with col2:
    ...
    df_price_plot = df_price_plot[df_price_plot['price'] < 1000]
